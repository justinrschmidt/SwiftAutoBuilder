...
/// The macro that generates a builder class for the type that the macro is attached to. The builder
/// class is generated in an extension of the attached type. The extension also provides conformance
/// to `Buildable` for the attached type.
///
/// The macro's work is divided into two main phases: the analysis phase and the generation phase.
///
/// The analysis phase inspects the declarations in the attached type. It extracts the information that
/// is needed to generate the builder class and passes that information into the generation phase.
/// If there are any issues that would prevent the builder class from being generated, those issues
/// are emitted from the analysis phase as compiler diagnostics and the generation phase is not run.
///
/// The generation phase assumes that the code of the attached type is well formed. It uses the
/// information emitted by the analysis phase to generate a builder class for the attached type.
///
...
public struct AutoBuilderMacro: ExtensionMacro {
    ...
    /// The list of `AutoBuilderExtensionGenerator`s that can generate a builder class
    /// from a declaration.
    ...
    // This method should only return an `ExtensionDeclSyntax` when `conformingTo` contains `Buildable`.
